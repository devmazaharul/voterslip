...
interface Voter {
  ...
  dateOfBirth: string;
  serialNumber: number;
  villageName: string; // ✅ Fixed: string, not number
  createdAt: string;
}
...
export default function AdminDashboard() {
  ...
  // ─── Village Select State (Modal er vitore) ───
    ...
    try {
      const res = await fetch(`/api/admin/${selectedVoter._id}`, {
        ...
        body: JSON.stringify({
          ...
          dateOfBirth: formData.dateOfBirth,
          serialNumber: parseInt(formData.serialNumber),
          villageName: formData.villageName, // ✅ Added
        }),
      });
      const data = await res.json();
      if (data.success) {
  ...
  return (
    <>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
          <div className="group bg-white/[0.02] backdrop-blur border border-white/[0.06] rounded-2xl p-4 hover:border-purple-500/20 transition-all duration-300">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-[10px] text-purple-400 bg-purple-500/10 px-2 py-0.5 rounded-full">Total</span>
            </div>
          ...
          <div className="group bg-white/[0.02] backdrop-blur border border-white/[0.06] rounded-2xl p-4 hover:border-emerald-500/20 transition-all duration-300">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-[10px] text-emerald-400 bg-emerald-500/10 px-2 py-0.5 rounded-full">Today</span>
            </div>
            <p className="text-2xl font-bold text-white">{stats?.todayAdded ?? "—"}</p>
          ...
          <div className="group bg-white/[0.02] backdrop-blur border border-white/[0.06] rounded-2xl p-4 hover:border-blue-500/20 transition-all duration-300">
            <div className="flex items-center justify-between mb-3">
              ...
              <span className="text-[10px] text-blue-400 bg-blue-500/10 px-2 py-0.5 rounded-full">Month</span>
            </div>
            <p className="text-2xl font-bold text-white">{stats?.thisMonthAdded ?? "—"}</p>
            ...
            <div className="flex items-center justify-between mb-3">
              <div className="w-9 h-9 bg-amber-500/10 rounded-xl flex items-center justify-center">
                <svg className="w-4 h-4 text-amber-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.8} d="M5.25 8.25h15m-16.5 7.5h15m-1.8-13.5l-3.9 19.5m-2.1-19.5l-3.9 19.5" />
                </svg>
              </div>
              <span className="text-[10px] text-amber-400 bg-amber-500/10 px-2 py-0.5 rounded-full">Latest</span>
            </div>
            <p className="text-2xl font-bold text-white">#{stats?.latestSerial ?? "—"}</p>
        ...
        <div className="bg-white/[0.02] backdrop-blur border border-white/[0.06] rounded-2xl overflow-hidden">
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-white/[0.06]">
                  ...
                  <th className="text-left text-[10px] font-medium text-gray-500 uppercase tracking-wider px-4 py-3">Date of Birth</th>
                  <th className="text-left text-[10px] font-medium text-gray-500 uppercase tracking-wider px-4 py-3">Age</th>
                  <th className="text-left text-[10px] font-medium text-gray-500 uppercase tracking-wider px-4 py-3">Added</th>
              ...
              <tbody className="divide-y divide-white/[0.03]">
                {loading ? (
                  ...
                  voters.map((voter) => (
                    <tr key={voter._id} className="hover:bg-white/[0.02] transition-colors group">
                      ...
                      <td className="px-4 py-3"><span className="text-xs text-gray-400">{calcAge(voter.dateOfBirth)} yrs</span></td>
                      <td className="px-4 py-3 text-xs text-gray-500">{formatDate(voter.createdAt)}</td>
                      <td className="px-4 py-3">
          ...
          <div className="md:hidden divide-y divide-white/[0.04]">
            {loading ? (
              ...
              voters.map((voter) => (
                <div key={voter._id} className="p-4 hover:bg-white/[0.02] transition-colors">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 mt-0.5 flex-wrap">
                          ...
                          <span className="text-[10px] text-gray-500">{calcAge(voter.dateOfBirth)} yrs</span>
                        </div>
                        <p className="text-[10px] text-gray-600 mt-0.5">DOB: {formatDate(voter.dateOfBirth)}</p>
                      </div>
                    </div>
      ...
      {/* ══════════════════════════════════════════════ */}
      {/* ═══ ADD / EDIT MODAL (with villageName) ═══ */}
      {/* ══════════════════════════════════════════════ */}
          ...
          <div className="relative w-full max-w-sm animate-[scaleIn_0.2s_ease]">
            ...
            <div className="relative bg-[#111118] border border-white/[0.08] rounded-2xl p-6">
              ...
              <div className="space-y-3.5">
                {/* Full Name */}
                <div>
                  <label className="block text-[10px] font-medium text-gray-400 mb-1.5 ml-1 uppercase tracking-wider">Full Name</label>
                  <input
                    type="text"
                    value={formData.name}
                  ...
                  <div className="relative">
                    ...
                    >
                      <span className="truncate">
                        {formData.villageName || "গ্রাম নির্বাচন করুন"}
                      </span>
                      ...
                      <div className="flex items-center gap-1.5 flex-shrink-0">
                        {formData.villageName && (
                          <span
                            onClick={(e) => {
                              e.stopPropagation();
                              setFormData({ ...formData, villageName: "" });
                            }}
                        ...
                        <svg
                          className={`w-4 h-4 text-gray-500 transition-transform duration-300 ${villageSelectOpen ? "rotate-180" : ""}`}
                        ...
                        <div className="absolute z-[80] mt-1.5 w-full animate-[dropIn_0.2s_ease]">
                          <div className="bg-[#0d0d14] border border-white/[0.08] rounded-xl shadow-[0_20px_60px_-15px_rgba(0,0,0,0.9)] overflow-hidden">
                            <div className="px-3 py-2 border-b border-white/[0.06]">
                              <p className="text-[10px] text-gray-500 uppercase tracking-wider">{VILLAGES_NAME.length} টি গ্রাম</p>
                            </div>
                              ...
                              {VILLAGES_NAME.map((village) => {
                                ...
                                return (
                                  <button
                                    key={village}
                                    type="button"
                                    onClick={() => handleModalVillageSelect(village)}
              ...
              <div className="flex items-center gap-2.5 mt-5">
                <button
                  onClick={() => { setModal(null); setVillageSelectOpen(false); }}
                ...
                <button
                  ...
                  disabled={formLoading || !formData.name || !formData.dateOfBirth || !formData.serialNumber || !formData.villageName}
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-xs animate-[scaleIn_0.2s_ease]">
            ...
            <div className="relative bg-[#111118] border border-white/[0.08] rounded-2xl p-6 text-center">
              ...
              <p className="text-sm text-white font-medium mb-1">{selectedVoter.name} (#{selectedVoter.serialNumber})</p>
              {selectedVoter.villageName && (
                <p className="text-[10px] text-blue-400 mb-4">📍 {selectedVoter.villageName}</p>
              )}
              <div className="flex items-center gap-2.5">
