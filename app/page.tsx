...
const formatDobBangla = (raw: string): string => {
    ...
    eng = eng.slice(0, 8); // DDMMYYYY → ৮ ডিজিট
    ...
    if (eng.length <= 2) {
        ...
        formatted = eng.slice(0, 2) + '/' + eng.slice(2, 4) + '/' + eng.slice(4);
    }
...
const VoterSearchForm: React.FC = () => {
    ...
    const handleSubmit = async (e: FormEvent) => {
        ...
        if (!dobPattern.test(dobEng)) {
            setError('জন্ম তারিখ অবশ্যই DD/MM/YYYY ফরম্যাটে দিতে হবে (যেমন: ০৫/১২/১৯৯৫)।');
            return;
        }
    ...
    const isDobValid = /^\d{2}\/\d{2}\/\d{4}$/.test(toEnglishDigits(formData.dob || ''));
                        ...
                        <div className="flex items-center justify-between gap-3">
                            <div>
                                ...
                                <p className="text-[13px] md:text-sm text-slate-700 mt-1">
                                    জন্ম তারিখ (DD/MM/YYYY) এবং গ্রাম নির্বাচন করে আপনার
                                    ভোটকেন্দ্রসহ অন্যান্য তথ্য দেখুন।
                                </p>
                            </div>
                      ...
                      ${
                          !canSubmit
                            ...
                            <p className="mt-2 text-[11px] text-slate-500">
                                ...
                                <span className="font-semibold text-emerald-700">০৫/১২/১৯৯৫</span>।
                            </p>
                        </div>
                                    ...
                                    {apiData.IsSuccess ? (
                                        apiData.Data.data.length > 0 ? (
                                            ...
                                            <div className="text-sm text-red-700 bg-red-50 border border-red-200 rounded-xl px-4 py-6 text-center">
                                                দুঃখিত, প্রদত্ত জন্ম তারিখ ও গ্রাম অনুযায়ী কোনো
                                                ভোটার খুঁজে পাওয়া যায়নি।
                                            </div>
                                        )
                                    ) : (
                                        <div className="text-sm text-red-700 bg-red-50 border border-red-200 rounded-xl px-4 py-6 text-center">
