...
const useCopy = () => {
  ...
  const copy = (text: string | number, id: string) => {
    navigator.clipboard.writeText(String(text));   // ✅ number safe
    setCopied(id);
...
const VoterCard: React.FC<{
  ...
  return (
    ...
    >
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0 pt-0.5">
            ...
            <div className="flex items-center gap-2 mt-2 flex-wrap">
              {/* ✅ Village badge */}
              <span className="text-[9px] font-bold text-purple-300 bg-purple-500/10 px-1.5 py-0.5 rounded-md border border-purple-500/15">
                📍 {voter.village}
              </span>
      ...
      {/* NID / Voter Number — ✅ number → string */}
                ...
                {!searched && !loading && (
                  <div className="text-center py-14 sm:py-20">
                    ...
                    <p className="text-[11px] text-gray-700 max-w-xs mx-auto">
                      গ্রাম ও জন্ম তারিখ দিয়ে ভোটার তথ্য খুঁজুন
                    </p>
                  </div>
                )}
                ...
                {loading && (
                  <div className="text-center py-14 sm:py-20">
                    ...
                    <p className="text-[11px] text-gray-600 mt-1">
                      অনুসন্ধান চলছে, অপেক্ষা করুন
                    </p>
                  </div>
                )}

                {/* ✅ Results — key={voter._id || voter.voterNumber} */}
                {searched && !loading && results.length > 0 && (
                  <div className="space-y-4 max-h-[700px] overflow-y-auto pr-1">
                    {results.map((voter, index) => (
                      <VoterCard
                        key={voter._id || voter.voterNumber || index}
                        voter={voter}
                        index={index}
                      />
                    ))}
